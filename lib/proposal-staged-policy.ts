...
/** Stable across JSONB object-key reordering when resuming an interrupted prepare. */
export function canonicalStagedContent(value: unknown): string {
  if (Array.isArray(value)) return '[' + value.map(canonicalStagedContent).join(',') + ']'
  if (value && typeof value === 'object') return '{' + Object.keys(value).sort().map(key => JSON.stringify(key) + ':' + canonicalStagedContent((value as Record<string, unknown>)[key])).join(',') + '}'
