/**
 * Module sync config from DB — merged with code-defined module list.
 * Spun-off repo URL is stored in module_sync_config and editable in Admin UI.
...
/** Return module list with spunOffRepoUrl from DB and suggestedSpunOffRepoUrl from GitHub API or convention. */
export async function getModulesWithConfig(): Promise<ModuleSyncEntry[]> {
  const dbConfig = await getModuleSyncConfigFromDb()
  ...
  return MODULE_SYNC_ENTRIES.map((m) => {
...
/** Upsert spun_off_repo_url for a module. */
export async function setModuleSpunOffRepoUrl(
  ...
  const validId = MODULE_SYNC_ENTRIES.some((m) => m.id === moduleId)
  if (!validId) {
...
/** Get a single module entry with DB override for diff use. */
