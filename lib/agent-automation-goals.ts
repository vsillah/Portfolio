...
export const AUTOMATION_GOAL_SEEDS: AutomationGoalSeed[] = [
  {
    id: 'client-journey-lifecycle-control-plane',
    tier: 1,
    title: 'Install the client journey lifecycle control plane',
    objective: 'Create the governed journey spine that tracks a person or account from social signal through lead, meeting, proposal, payment, delivery, retention, and winback.',
    workflowFamily: 'client_journey',
    automationLevel: 'approval_gated',
    ownerAgentKey: 'automation-systems',
    collaboratorAgentKeys: ['chief-of-staff', 'inbox-follow-up', 'meeting-intake-follow-up', 'proposal-business-model'],
    sourceRoutes: ['/admin/agents', '/admin/outreach', '/admin/meetings', '/admin/sales', '/admin/client-projects'],
    sourceDocs: ['docs/client-journey-agentic-goals.md', 'docs/admin-sales-lead-pipeline-sop.md', 'docs/agent-operations-roadmap.md'],
    n8nWorkflows: ['WF-000', 'WF-CLG-003', 'WF-WRM-001', 'WF-CAL', 'WF-FUP'],
    approvalGate: 'Journey stage updates can prepare internal work automatically; outbound sends, calendar changes, proposal terms, payments, client-visible updates, and winback offers require approval.',
    nextAction: 'Define the lifecycle stage model and event router before connecting more autonomous actions.',
    requiresNewWorkflow: true,
    tasks: [
      {
        title: 'Define the journey stage state machine',
        objective: 'Create the canonical lifecycle states, stage transition rules, required evidence, and approval boundaries for each customer journey phase.',
        ownerAgentKey: 'automation-systems',
        priority: 'high',
        expectedFiles: ['docs/client-journey-agentic-goals.md', 'lib/agent-automation-goals.ts', 'app/admin/agents'],
        acceptanceCriteria: ['Every lifecycle stage has an owner agent.', 'Each transition lists required evidence and approval gates.', 'Existing Portfolio routes are mapped before any new surface is proposed.'],
        riskNotes: 'Do not create a parallel CRM or duplicate source of truth; this is a control plane over existing Portfolio records.',
        requiresApproval: true,
        progressWeight: 2,
      },
      {
        title: 'Route journey events into Agent Ops',
        objective: 'Normalize social engagement, replies, forms, meetings, proposal, payment, project, subscription, and cancellation events into Agent Ops work items or approval packets.',
        ownerAgentKey: 'chief-of-staff',
        priority: 'high',
        expectedFiles: ['app/api/admin/agents', 'app/admin/agents/coordination', 'lib/agent-work-items.ts'],
        acceptanceCriteria: ['Events preserve source IDs and trace links.', 'Risky next actions become approval packets.', 'Low-confidence routing becomes a Mission Control exception instead of silent automation.'],
        riskNotes: 'External side effects must stay blocked until the approval packet is explicitly accepted.',
        requiresApproval: true,
        progressWeight: 2,
      },
    ],
  ...
  {
    id: 'campaign-experimentation-ab-optimizer',
    tier: 1,
    title: 'Run governed campaign experimentation and A/B optimization',
    objective: 'Create a controlled AutoResearch loop for campaign variants across social posts, outreach drafts, follow-ups, and offer language, then use engagement and conversion data to recommend the next variant.',
    workflowFamily: 'campaign_experimentation',
    automationLevel: 'approval_gated',
    ownerAgentKey: 'strategic-narrative',
    collaboratorAgentKeys: ['voice-content-architect', 'research-source-register', 'inbox-follow-up', 'chief-of-staff'],
    sourceRoutes: ['/admin/social-content', '/admin/outreach', '/admin/analytics', '/admin/value-evidence', '/admin/agents/coordination'],
    sourceDocs: ['docs/client-journey-agentic-goals.md', 'docs/linkedin-voice.md', 'docs/agentic-content-challenger-loop.md'],
    n8nWorkflows: ['WF-SOC-001', 'WF-SOC-002', 'WF-CLG-003', 'WF-GDR'],
    approvalGate: 'Variant creation, publishing, outbound sends, and scaling decisions require approval; metric collection and internal analysis can run automatically.',
    nextAction: 'Define the campaign packet format: hypothesis, variants, channels, audience, metrics, decision rule, challenger findings, and next recommendation.',
    requiresNewWorkflow: true,
    tasks: [
      {
        title: 'Create the campaign experiment packet',
        objective: 'Define the packet fields for campaign hypotheses, approved variants, audience, channel, metrics, launch boundary, and decision criteria.',
        ownerAgentKey: 'strategic-narrative',
        priority: 'high',
        expectedFiles: ['docs/client-journey-agentic-goals.md', 'lib/agent-automation-goals.ts', 'app/admin/social-content'],
        acceptanceCriteria: ['Each variant has a campaign ID, source packet, channel, audience, and approval status.', 'Metrics include both engagement and conversion signals.', 'The packet records when a variant is retired, adapted, or scaled.'],
        riskNotes: 'Campaign tests must not become blind growth hacking; voice, source support, privacy, and approval gates remain active.',
        requiresApproval: true,
        progressWeight: 2,
      },
      {
        title: 'Add challenger-reviewed experiment recommendations',
        objective: 'Make AutoResearch compare campaign performance and produce recommendation packets checked by a challenger before human approval.',
        ownerAgentKey: 'research-source-register',
        priority: 'high',
        expectedFiles: ['app/admin/analytics', 'app/admin/agents/coordination', 'lib/social-engagement.ts'],
        acceptanceCriteria: ['Recommendations cite the metric window and sample size.', 'Weak, overfit, or misleading conclusions are flagged.', 'The next-round variant cannot publish or send without approval.'],
        riskNotes: 'Avoid optimizing only for reactions when booked calls, qualified replies, and client conversion are the real business outcomes.',
        requiresApproval: true,
        progressWeight: 2,
      },
    ],
  ...
  {
    id: 'mobile-chat-command-approval-surface',
    tier: 1,
    title: 'Control the business lifecycle from mobile or chat',
    objective: 'Let Shaka surface journey actions through Standup, Slack/mobile, or chat so Vambah can approve, repair, hold, or delegate without opening every Portfolio screen.',
    workflowFamily: 'command_surface',
    automationLevel: 'approval_gated',
    ownerAgentKey: 'chief-of-staff',
    collaboratorAgentKeys: ['automation-systems', 'inbox-follow-up', 'risk-compliance-intelligence'],
    sourceRoutes: ['/admin/agents/standup', '/admin/agents/coordination', '/admin/agents/runs', '/admin/outreach', '/admin/client-projects'],
    sourceDocs: ['docs/client-journey-agentic-goals.md', 'docs/agent-operations-roadmap.md'],
    n8nWorkflows: ['WF-SLK', 'WF-FUP'],
    approvalGate: 'Chat and mobile commands can approve only the next explicitly described gate; sends, publishing, payments, proposal terms, and client-visible updates remain separately gated.',
    nextAction: 'Define the command packet contract so every mobile/chat action includes context, evidence, risk, and rollback path.',
    requiresNewWorkflow: false,
    tasks: [
      {
        title: 'Define command packet actions',
        objective: 'Standardize approve, send back, hold, delegate, schedule draft, publish draft, send follow-up, and generate package commands with clear authority boundaries.',
        ownerAgentKey: 'chief-of-staff',
        priority: 'high',
        expectedFiles: ['app/admin/agents/standup', 'app/admin/agents/coordination', 'lib/chief-of-staff-chat.ts'],
        acceptanceCriteria: ['Every command includes subject, evidence, recommended action, risk, and allowed side effect.', 'Commands do not infer approval for a broader action than the packet describes.', 'A held command leaves a traceable reason.'],
        riskNotes: 'A one-step approval should be convenient, not ambiguous; the packet has to name exactly what will happen.',
        requiresApproval: true,
        progressWeight: 2,
      },
      {
        title: 'Route lifecycle exceptions to mobile review',
        objective: 'Surface reply-needed, meeting-needed, proposal-ready, client-update-ready, campaign-decision, cancellation, and winback packets where Vambah can act quickly.',
        ownerAgentKey: 'automation-systems',
        priority: 'high',
        expectedFiles: ['app/admin/agents/standup', 'app/admin/agents/runs', 'lib/agent-slack-notification-sweep.ts'],
        acceptanceCriteria: ['High-priority lifecycle packets have a mobile-safe summary.', 'Approval actions link back to the full trace.', 'No raw private source text is sent to public or mobile notification surfaces.'],
        riskNotes: 'Summaries must be sanitized and source-linked instead of exposing raw private transcripts or private operational logs.',
        requiresApproval: true,
        progressWeight: 2,
      },
    ],
