...
| Credential provisioning and sync | `docs/credential-management-system.md`, `/admin/credentials`, `scripts/credential-broker.ts`, new client-stack setup | Stripe Projects, Infisical integrations, 1Password service accounts, provider-native CLIs, Vercel integrations | secret safety, agent ergonomics, source-of-truth fit, rotation support, auditability, portability | candidate passes `docs/stripe-projects-bakeoff.md` without exposing values or replacing governance sources |
