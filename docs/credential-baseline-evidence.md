...
| `APIFY_API_TOKEN` | 2026-05-25 | Approved staging-only rotation: new limited-permission Apify token created with 2026-08-23 expiration, Infisical staging `/portfolio` secret synced, n8n `ATAS Staging` variable `STAG_APIFY_TOKEN` updated via API, local staging aliases updated, Apify actor/run/task/storage smoke endpoints returned 200, and the old `Access for ATAS Staging Environment` token was deleted after verification. |
| `SUPABASE_SERVICE_ROLE_KEY` | 2026-05-26 | Approved staging-only Supabase secret key rotation: Supabase secret API key `portfolio_staging_rotation_20260526` created, prior exposed/unused staging keys deleted, Infisical staging `/portfolio`, n8n `ATAS Staging`, local staging aliases, and `portfolio-staging` Vercel runtime sink synced, staging redeployed, hash-only verification matched, synthetic n8n ATAS Staging Supabase smoke execution `15228` succeeded, and the old staging `default` secret key was operator-confirmed deleted after post-deletion credential report/smoke validation. |
| `STAG_SUPABASE_SERVICE_ROLE_KEY` | 2026-05-26 | Approved staging-only Supabase secret key rotation: n8n `ATAS Staging` variable updated via API with the same replacement key as the staging app-side Supabase secret; hash-only verification matched, synthetic n8n ATAS Staging Supabase smoke execution `15228` succeeded, and the old staging `default` Supabase secret key was operator-confirmed deleted after post-deletion credential report/smoke validation. |
