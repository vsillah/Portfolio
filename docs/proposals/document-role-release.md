...
- Actual local conversation route with all API writes intercepted and synthetic PDF bytes, at 360/390/768/1440. Role selection, ordinary type, upload, same-request retry/reload, completed receipt/Done, document read link, supporting deletion, locked role options and Cancel exercised. The attachment overlay was fixed above the existing drawer. Public proposal routes were not opened for QA.
...
The SQL harness refuses a different container name or enabled networking before resetting its synthetic schema. Final MP4s decode successfully with FFmpeg: desktop 21.8 seconds, mobile 20.4 seconds. Changed-file ESLint and git diff --check passed.
