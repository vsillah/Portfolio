...
4. **Outreach generation** — (a) **n8n:** Admin clicks **Generate Email** on a lead (or n8n runs after enrichment) → `N8N_CLG002_WEBHOOK_URL` (WF-CLG-002) generates drafts into `outreach_queue`. (b) **In-app:** Admin clicks **Draft in app** → `POST .../generate-in-app` runs OpenAI and inserts `outreach_queue` directly (no CLG-002). Use n8n when the full automation chain is desired; use in-app when n8n is down or you want an immediate draft from current DB context.
   - **Why n8n can show "Succeeded" while All Leads still "queues" or shows Retry:** The app sets `last_n8n_outreach_status` to **pending** when the CLG-002 webhook is fired. The pill only flips to **success** when a new **email** `outreach_queue` row is inserted (or the completion callback runs with `status: success`). n8n marks the **run** Succeeded as soon as no node errors — including when an **IF** routes to a branch that does **not** create a row (e.g. "Already Contacted?" is false because `outreach_status` is not `not_contacted`, so no email is written). In that case you must end that branch with an **HTTP node** to `POST /api/webhooks/n8n/outreach-generation-complete` with `status: failed` (and a clear `error_message` such as *Skipped: not not_contacted*) so the app clears the pending state. If you *should* be allowed to send for that case, change the IF / routing in WF-CLG-002 instead. Without a row **or** that callback, the UI stays pending until the in-app long wait or DB reconcile time out to **failed**.
