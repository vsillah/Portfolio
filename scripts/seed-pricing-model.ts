...
const SERVICES = [
  ...
  },
  {
    ...
    display_order: 17,
    is_featured: true,
    topics: ['AI Fundamentals', 'Prompt Engineering', 'Business Applications', 'Hands-On Exercises'],
  ...
  },
  {
    ...
    display_order: 18,
    is_featured: true,
    topics: ['Production AI', 'Model Deployment', 'Hands-On Projects', 'Certification'],
    deliverables: ['Masterclass access', 'Project templates', 'Code repositories', 'Certificate'],
  },
  {
...
async function seed() {
  ...
  // Services (key build services + training courses)
  const serviceRoles: Array<{ title: string; role: string; perceivedValue: number; retailPrice?: number; offerPrice?: number; dreamOutcome: string; likelihood: number; timeReduction: number; effortReduction: number }> = [
    { title: 'AI Training Course — Self-Paced', role: 'core_offer', perceivedValue: 1500, retailPrice: 497, offerPrice: 297, dreamOutcome: 'Master AI tools and apply them to your business immediately', likelihood: 7, timeReduction: 5, effortReduction: 6 },
    { title: 'Premium AI Masterclass', role: 'core_offer', perceivedValue: 2500, retailPrice: 997, offerPrice: 497, dreamOutcome: 'Build and deploy production AI solutions with confidence', likelihood: 7, timeReduction: 5, effortReduction: 6 },
  ...
  for (const sr of serviceRoles) {
    ...
    const { error } = await supabase.from('content_offer_roles').insert({
      ...
      retail_price: sr.retailPrice ?? sr.perceivedValue,
      offer_price: sr.offerPrice ?? sr.perceivedValue,
