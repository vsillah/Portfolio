...
const out = path.join(root, 'docs/qa/sms-consent-policy')
const raw = path.join(root, 'test-results/sms-consent-policy')
...
try {
  for (const width of [1440, 390, 360, 768]) {
    ...
    await expect(page.getByRole('heading', { name: 'AmaduTown SMS program', exact: true })).toBeVisible()
