...
export const warmSlackSendApprovalQaRelationshipPacket = {
  ...
  manualSocialHandoff: {
    ...
    contactId: String(WARM_SLACK_SEND_APPROVAL_QA_CONTACT_ID),
    contactName: warmSlackSendApprovalQaLead.name,
    state: 'ready',
    ...
    channels: [
      {
        ...
        blocker: null,
        preview:
          'Hi Amina, I was thinking about the synthetic warm follow-up context. I am reviewing AmaduTown operating-system support and would value a short manual conversation if useful.',
        ...
        idempotency: {
          messageVersionKey: 'warm-outreach:manual-message-version:v1:qa-linkedin-42',
          manualHandoffKey: 'warm-outreach:manual-handoff:v1:qa-linkedin-42',
          manualEvidenceKey: 'warm-outreach:manual-evidence:v1:qa-linkedin-42',
        ...
        evidencePolicy: {
          ...
          detail:
            'Record only that the manual handoff happened, the channel, timestamp, and a non-sensitive operator note.',
        },
      },
      {
        channel: 'facebook',
        ...
        blocker: null,
        preview:
          'Hi Amina, hope you are well. The synthetic warm follow-up context came back to mind while reviewing Portfolio QA. I would value a quick manual conversation if useful.',
        maxRecommendedCharacters: 360,
        checklist: [
          { key: 'relationship_basis', label: 'Warm basis reviewed', status: 'ready' },
          { key: 'suppression', label: 'Suppression checked', status: 'ready' },
          { key: 'copy_manually', label: 'Copy into the channel manually', status: 'manual_required' },
        ...
        idempotency: {
          messageVersionKey: 'warm-outreach:manual-message-version:v1:qa-facebook-42',
          manualHandoffKey: 'warm-outreach:manual-handoff:v1:qa-facebook-42',
          manualEvidenceKey: 'warm-outreach:manual-evidence:v1:qa-facebook-42',
          duplicateScope: 'contact_channel_message_version',
        },
        executionBoundary: {
          manualOnly: true,
      ...
      },
      {
        ...
        blocker: null,
        preview:
          'Hi Amina, this is Vambah. Reaching out from the synthetic warm follow-up context. If useful, I would like to compare notes on a small operations review.',
        ...
        idempotency: {
          messageVersionKey: 'warm-outreach:manual-message-version:v1:qa-phone-contact-42',
          manualHandoffKey: 'warm-outreach:manual-handoff:v1:qa-phone-contact-42',
          manualEvidenceKey: 'warm-outreach:manual-evidence:v1:qa-phone-contact-42',
          duplicateScope: 'contact_channel_message_version',
        },
        executionBoundary: {
          manualOnly: true,
    ...
    auditState: {
      ...
      detail:
        'Synthetic manual handoff is copy-and-record only; no provider automation is enabled.',
    },
    executionBoundary: {
      manualCopyOnly: true,
      providerCallsEnabled: false,
  ...
  smsReadiness: {
    ...
    channel: 'phone_contact',
    state: 'manual_ready',
    label: 'SMS draft ready for manual approval',
    phoneReadiness: {
      present: true,
      source: 'contact_submissions.phone_number',
      provenance: 'Synthetic QA phone number is present on the Portfolio contact record.',
      rawPhoneReturned: false,
    },
    relationshipRationale: {
      status: 'present',
      basis:
        'Synthetic test-owned relationship evidence is present for the QA route so the operator can review manual SMS readiness without private data or provider calls.',
      sourceCount: 2,
      signalCount: 3,
      ...
      checks: [
        {
          ...
          status: 'passed',
          detail: 'Portfolio has a phone number on the contact record.',
        },
        {
          key: 'phone_provenance',
          label: 'Phone source provenance',
          status: 'review_required',
          detail: 'Confirm the contact record source is appropriate for manual SMS.',
        },
        {
          key: 'relationship_basis',
          label: 'Relationship rationale',
          status: 'passed',
          detail: 'Portfolio has relationship evidence beyond the contact row.',
        },
        {
          key: 'suppression',
          label: 'Suppression',
          status: 'passed',
          detail: 'No DNC, unsubscribe, or removed-state blocker is recorded.',
        },
        {
          key: 'opt_out',
          label: 'Opt-out sensitivity',
          status: 'review_required',
          detail: 'Operator must stop immediately if the contact declines or opts out.',
        },
        {
          key: 'manual_only',
          label: 'Manual only',
    ...
    draft: {
      templateFamily: 'prior_email_follow_up',
      templateLabel: 'Follow-up after prior email',
      selectionReason: 'Prior email or follow-up evidence is the strongest local signal.',
      preview:
        'Hi Amina, following up on my email about Portfolio QA review. No pressure, but is this still worth a quick look?',
      guidance: [
  ...
  responseMonitoring: {
    ...
    responseDigest: {
      ...
      classification: {
        responseClass: null,
        label: 'No response classified',
        confidence: null,
        sourceId: null,
      },
      nextBestAction: {
        label: 'Capture response evidence',
        description: 'Manual or approved provider-shaped response evidence is required before drafting a follow-up.',
        priority: 'medium',
      ...
      followUpDraft: {
        ...
        sourceId: null,
        idempotencyKey: null,
        detail: 'No local reply evidence is available yet, so no follow-up draft is ready.',
      },
      suppressionProposal: {
        state: 'not_applicable',
        actionLabel: 'No suppression proposal',
        reason: 'No hold, opt-out, wrong-channel, or sensitive response evidence is recorded.',
        idempotencyKey: null,
        requiresHumanApproval: true,
    ...
    gmailResponseImportReadiness: {
      ...
      activationReadiness: {
        ...
        missingScopes: [],
        blockedReasons: ['Live Gmail response reads are disabled by default.'],
        nextOperatorAction:
          'Use mocked dry-run planning only; live Gmail reads require a future captain-authorized activation switch.',
        recoveryPath:
          'Keep the provider-read path off; review mock candidates and request a separate captain gate for any live Gmail read.',
        futureLiveSwitch: {
        ...
        gateRows: [
          {
            ...
            label: 'Mock import',
            state: 'ready',
            detail: 'Dry-run planning can use mocked Gmail payloads and local Portfolio rows.',
            nextAction: 'Run the admin dry-run route with fixture payloads.',
          },
          {
            key: 'live_import',
            label: 'Live import',
          ...
          },
          {
            ...
            label: 'Provider',
            state: 'ready',
            detail: 'Gmail response-import provider configuration is present.',
            nextAction: 'Keep provider reads disabled.',
          },
          {
            key: 'gmail_token',
            label: 'Gmail token',
            state: 'ready',
            detail: 'A connected Gmail token is present in readiness metadata.',
            nextAction: 'Do not read Gmail until a separate activation gate exists.',
          },
          {
            key: 'gmail_scope',
            label: 'Gmail scope',
            state: 'ready',
            detail: 'Stored scope evidence includes gmail.readonly.',
            nextAction: 'Keep mock planning separate from live provider authority.',
          },
          {
            key: 'manual_recovery',
            label: 'Manual recovery',
            state: 'ready',
            detail: 'No duplicate, ambiguous, suppressed, or existing-evidence blocker is active.',
            nextAction: 'Review ready candidates through the existing warm response lifecycle.',
          },
        ],
      },
      matchBasis: [
