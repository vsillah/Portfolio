...
function PrepareProposalContent() {
  ...
  return <main className="max-w-4xl mx-auto p-4 sm:p-8 text-foreground">
    ...
    {result && <section className="my-8 space-y-4 rounded-lg border border-radiant-gold/20 p-4">
      ...
      <button className="inline-flex items-center gap-2 rounded-lg border border-radiant-gold/20 bg-muted px-4 py-3 text-foreground disabled:text-muted-foreground" disabled={busy} onClick={() => loadPackage(result.proposalId).catch(e => setMessage(e.message))}><RefreshCw size={16} />Refresh status</button>
