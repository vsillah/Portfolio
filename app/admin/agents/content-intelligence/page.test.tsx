...
describe('ContentIntelligencePage', () => {
  ...
  it('shows research packets and central Shaka insights', async () => {
    ...
    expect(screen.getByText('1–4 of 5')).toBeInTheDocument()
    expect(screen.queryByText('Case study proof drop')).not.toBeInTheDocument()
    ...
    expect(screen.getByText('Showing 1-1 of 2 backlog items. Search also matches 2 ranked opportunities and connected release rows.')).toBeInTheDocument()
    ...
    expect(within(backlogActionList).getByText('Page 1 of 2')).toBeInTheDocument()
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Next' }))
    expect(within(backlogActionList).getByText('Page 2 of 2')).toBeInTheDocument()
    ...
    expect(screen.getByText('Showing 1-1 of 1 backlog item. Search also matches 0 ranked opportunities and connected release rows.')).toBeInTheDocument()
    expect(screen.getAllByText('What breaks first when AI gets faster?').length).toBeGreaterThan(0)
    ...
    expect(screen.getByText('Showing 1-1 of 1 backlog item. Search also matches 1 ranked opportunity and connected release rows.')).toBeInTheDocument()
    expect(screen.getByText('Name the receipt path before asking operators where trust breaks.')).toBeInTheDocument()
