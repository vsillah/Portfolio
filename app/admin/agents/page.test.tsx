...
describe('AgentOperationsPage mission control landing', () => {
  ...
  it('loads the mission control landing with primary actions and status hierarchy', async () => {
    ...
    expect(screen.getByRole('tab', { name: /Morning review/i })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText('Manual pollers. Each run writes a trace to Run Console.')).toBeInTheDocument()
