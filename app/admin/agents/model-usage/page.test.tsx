...
describe('ModelUsagePage', () => {
  ...
  it('renders model usage totals, heatmap, recommendations, and transactions', async () => {
    ...
    expect(screen.getByText('Reviewed source file')).toBeInTheDocument()
    expect(screen.getByText('Client-safe usage export')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Stage source file/i })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Copy client-safe JSON/i })).toBeInTheDocument()
