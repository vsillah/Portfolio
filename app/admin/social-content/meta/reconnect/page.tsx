...
import { startMetaReconnect } from '@/lib/meta-reconnect-client'

type ReconnectState =
  | { status: 'loading'; message: string }
  | { status: 'error'; message: string }

export default function MetaReconnectPage() {
  const [state, setState] = useState<ReconnectState>({
    status: 'loading',
    message: 'Preparing the secure Meta reconnect flow...',
  })
  ...
  useEffect(() => {
    ...
    startMetaReconnect({
      ...
      fetchAuthUrl: (accessToken) => fetch('/api/auth/meta', {
        headers: {
      ...
      if (!mounted) return
      setState({ status: 'error', message: 'Meta connection could not start.' })
    })
  ...
  return (
    <main className="min-h-screen bg-background px-6 py-16 text-foreground">
      <section className="admin-console-card mx-auto max-w-2xl rounded-xl p-8">
        ...
        <div className="admin-console-eyebrow mb-3">Meta Provider</div>
        <h1 className="text-2xl font-semibold">Reconnect Instagram and Facebook</h1>
