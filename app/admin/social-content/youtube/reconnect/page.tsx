...
import { startYouTubeReconnect } from '@/lib/youtube-reconnect-client'

type ReconnectState =
  | { status: 'loading'; message: string }
  | { status: 'error'; message: string }

export default function YouTubeReconnectPage() {
  const [state, setState] = useState<ReconnectState>({
    status: 'loading',
    message: 'Preparing the secure YouTube reconnect flow...',
  })
  ...
  useEffect(() => {
    ...
    startYouTubeReconnect({
      ...
      fetchAuthUrl: (accessToken) => fetch('/api/auth/youtube', {
        headers: {
      ...
      if (!mounted) return
      setState({ status: 'error', message: 'YouTube connection could not start.' })
    })
  ...
  return (
    <main className="min-h-screen bg-background px-6 py-16 text-foreground">
      <section className="admin-console-card mx-auto max-w-2xl rounded-xl p-8">
        ...
        <div className="admin-console-eyebrow mb-3">YouTube Provider</div>
        <h1 className="text-2xl font-semibold">Reconnect YouTube</h1>
