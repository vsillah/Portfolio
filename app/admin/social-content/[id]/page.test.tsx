...
describe('SocialContentDetailRoute visual production review', () => {
  ...
  it('keeps approved copy locked while exposing visual production actions', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Human review: Approved' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Challenger: Pending' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Chronicle: Pending' })).toBeInTheDocument()
