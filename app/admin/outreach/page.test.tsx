...
describe('OutreachAdminPage deep links', () => {
  ...
  it('shows a compact warm planning backlog on the planning view route', async () => {
    ...
    const campaignContext = within(planningBacklog).getByLabelText('Warm planning campaign context')
    expect(within(campaignContext).queryByText(/This week Sep/)).not.toBeInTheDocument()
    expect(within(campaignContext).queryByText(/Sep \d+-Sep \d+/)).not.toBeInTheDocument()
    expect(within(campaignContext).queryByText('Today / This week')).not.toBeInTheDocument()
    expect(within(campaignContext).queryByText(/^Today Sep/)).not.toBeInTheDocument()
    expect(within(campaignContext).queryByText(/^Week Sep/)).not.toBeInTheDocument()
    expect(within(planningBacklog).getAllByText(/Whisper-to-shout launch/).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getByText(/The backlog uses the existing .* content-calendar milestone/)).toBeInTheDocument()
    expect(within(planningBacklog).getByText('Calendar LinkedIn')).toBeInTheDocument()
    expect(within(planningBacklog).getAllByText(/Cadence Campaign day/).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getAllByText(/Gate copy review/).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getAllByText(/Proof /).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getAllByText(/Source HubSpot social calendar template/).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getByText(/Source Whisper-to-shout launch content calendar template/)).toBeInTheDocument()
    ...
    expect(within(planningBacklog).getAllByText(/Calendar (Tease|Teach|Proof|Offer)/).length).toBeGreaterThan(0)
