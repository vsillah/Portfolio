...
describe('OutreachAdminPage deep links', () => {
  ...
  it('shows a compact daily warm shortlist on the warm outreach route', async () => {
    ...
    expect(within(planningBacklog).getByText(/The backlog turns the .* campaign signal/)).toBeInTheDocument()
    expect(within(planningBacklog).getAllByText(/Daily from (Tease|Teach|Proof|Offer) calendar/).length).toBeGreaterThan(0)
    expect(within(planningBacklog).getAllByText(/Daily recommendations rank existing Lead Pipeline rows/).length).toBeGreaterThan(0)
    ...
    expect(within(planningBacklog).getByText('Calendar basis')).toBeInTheDocument()
    ...
    expect(within(planningBacklog).getAllByText(/Basis:/).length).toBeGreaterThan(0)
