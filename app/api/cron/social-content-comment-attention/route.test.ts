...
describe('/api/cron/social-content-comment-attention', () => {
  ...
  it('does not report provider read side effects for setup failures before adapter delegation', async () => {
    mocks.runSocialCommentAttentionYouTubeRefresh.mockResolvedValueOnce({
      ok: false,
      status: 'failed',
      dryRun: false,
      selectedCount: 1,
