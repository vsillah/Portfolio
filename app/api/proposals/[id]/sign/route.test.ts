...
function makeRequest(
  ...
  return new NextRequest('http://localhost/api/proposals/prop-1/sign', {
    method: 'POST',
...
function mockProposalLookup(
  proposal: { id: string; status: string; valid_until: string | null } | null,
...
describe('POST /api/proposals/[id]/sign', () => {
  ...
  it('requires signed_by_name', async () => {
    const response = await POST(makeRequest({ signature_data: 'data:image/png;base64,abc' }), params())
  ...
  it('persists trimmed signer name, IP, and optional signature data', async () => {
    ...
    const response = await POST(
      makeRequest(
        { signed_by_name: '  Ada Client  ', signature_data: 'sig-bytes' },
