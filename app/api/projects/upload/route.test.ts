...
describe('POST /api/projects/upload', () => {
  ...
  it('rejects unauthenticated callers before reading the file', async () => {
    ...
    const response = await POST(
      makeRequest({ file: new File(['x'], 'brief.pdf', { type: 'application/pdf' }) }),
    )
  ...
  it('returns 400 when no file is provided', async () => {
    const response = await POST(makeRequest({ projectId: '41' }))
  ...
  it('stores files under project-{id}/ when projectId is present', async () => {
    const file = new File(['deck'], 'Brief.PDF', { type: 'application/pdf' })
    const response = await POST(makeRequest({ file, projectId: '41' }))
    const body = await response.json()

    expect(response.status).toBe(200)
    expect(body.success).toBe(true)
    expect(body.file_path.startsWith('project-41/')).toBe(true)
  ...
  it('falls back to the uploads/ tree when projectId is omitted', async () => {
    const file = new File(['deck'], 'brief.pdf', { type: 'application/pdf' })
  ...
  it('returns the storage error message on the 500 path', async () => {
    mocks.upload.mockResolvedValue({ data: null, error: { message: 'quota exceeded' } })
