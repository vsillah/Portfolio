...
function getRequest() {
  return new NextRequest('http://localhost/api/music/7')
}
...
describe('GET /api/music/[id]', () => {
  ...
  it('returns unpublished rows including file_path without auth', async () => {
    const row = { id: 7, title: 'Draft', is_published: false, file_path: 'music/draft.mp3' }
  ...
  it('maps PGRST116 to 404', async () => {
    ...
    await expect(response.json()).resolves.toEqual({ error: 'Music not found' })
  })

  it('returns the database error message on unexpected failures', async () => {
    const single = vi.fn().mockResolvedValue({
      data: null,
      error: { code: 'XX000', message: 'music detail failed' },
    })
    mocks.from.mockReturnValue({
      select: vi.fn().mockReturnValue({ eq: vi.fn().mockReturnValue({ single }) }),
    })
    ...
    await expect(response.json()).resolves.toEqual({ error: 'music detail failed' })
  })
})
...
describe('PUT /api/music/[id]', () => {
  ...
  it('writes only provided fields plus updated_at', async () => {
    const updated = { id: 7, title: 'Renamed', is_published: false }
...
describe('DELETE /api/music/[id]', () => {
  ...
  it('deletes the row and returns JSON success', async () => {
    ...
    expect(mocks.from).toHaveBeenCalledWith('music')
    expect(eq).toHaveBeenCalledWith('id', '7')
  })
})
